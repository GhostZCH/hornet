...
+ 多层存储
  + 充分利用不同设备的分层存储（内存，SSD，HDD），实现性能和成本的平衡，重启不丢失数据
  + 使用共享内存设备作为内存缓存重启程序不重启机器的情况下
+ 足够用的性能
  + QPS：4k资源 200K+，16K资源 50K+ 
  + 热点数据分散缓存
  + 加载启动小于10s
  + 高级淘汰算法提高命中率（目前使用ARC，后续改进）
  + 适配新的硬件，多线程，大内存，SSD，发挥多核等硬件优势,CPU，内存，SSD都升级很快，价格降低很多，不需要过度关注磁盘性能和计算速度，功能优先
+ 大容量
  + 单个资源可以到数G
  + 支持1000W资源
  + 可以管理数T的资源
+ 丰富的功能
  + 支持回源
  + 热点数据分散缓存（TODO）
  + 对range格式良好的支持，分片对齐（TODO）
  + 丰富而快速的删除功能（/用户组/用户/域/子域，tag，前缀，后缀，通配符）
+ 透明或者半透明
  + 可以无感的放到调用者和被调用者之间完成缓存动作（比如入口的nginx和业务服务器中间）
  + 通过http头覆盖默认配置提供更好的服务
+ 分布式
  + 分为三种角色：queue,proxy,cache 
  + 三种角色都支持横向扩展
  + 内嵌代理逻辑proxy（TODO）
  + 内嵌管理逻辑queue，实现服务发现，配置中心等逻辑，查看集群状态，下发命令的操作（TODO）
+ 简单
  + 甚至牺牲少许性能和功能换取简单的代码逻辑
  + 使用golang作为开发语言，大量引用成熟的库，通过合理的数据结构设计和组件使用实现功能
+ 易用
  + 尽量简化配置过程不让用户思考，智能填充业务参数，
  + 根据资源数量自动调整桶数量（TODO）
  + 根据资源大小自动分片（TODO）
  + 默认使用合理的分片大小
