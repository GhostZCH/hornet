...
hornet是一个支持大量内容和丰富的删除策略的使用分层存储的分布式高性能缓存系统。内存缓存速度快但容量有限且不支持对象和元数据的持久化，磁盘存储容量大支持持久化但性能较差，hornet将不同热度和大小的对象分层缓存包含了两种存储的优势。单节点设计用于缓存占用数TB空间的数千万对象，每个对象可以从几KB到几个G。针对如此大量的缓存支持多种批量删除策略，如按照路径，标签或者正则。此外，如其他缓存系统一样支持缓存数据的自动替换和过期。

目前系统主要支持http协议，使用轻量化的设计理念方便维护和二次开。使用整块空间存储对象并进行过期管理，减少内存碎片和海量小文件造成的问题。系统采用“半透明”方式进行缓存更新，将系统部署在调用方和被调用方中间，调用方访问hornet后在缓存失效时自动到被调用方取数据，对于调用方和被调用方都可以无视缓存的的存在称为“透明”，调用方和被调用方需要进行少量的接口适配（增加几个http头）所以我们称之为“半透明”。

## 背景

hornet开发的目的主要是为了解决CDN缓存系统应对的访问量大，存储量大，数量多，分布式，需要支持多种删除策略等挑战。在生产环境和测试环境上使用和测试了多种缓存引擎后，发现已有的成品都有些不如人意之处（详见下表）。于是自己动手开发新引擎，希望新的引擎能够“灵巧迅猛，群起而战”，于是便命名为Hornet(黄蜂)。

https://www.jianshu.com/p/2fd16f07202d
